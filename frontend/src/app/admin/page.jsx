...
export default function AdminTicketsPage() {
  ...
  useEffect(() => {
    socket.on("nuevoTicket", (ticket) => {
      ...
      setNotificacion({
        mensaje: `Nuevo ticket de ${ticket.departamento}`,
        prioridad: ticket.prioridad,
        urgente: ticket.urgencia_detectada === 1
      });
    ...
    socket.on("ticketActualizadoGlobal", (ticket) => {
      setTickets((prev) => {
        // Si ahora está Cerrado → quitarlo
        if (ticket.estado === "Cerrado") {
          return prev.filter((t) => t.id !== ticket.id);
        }

        // Si no está cerrado → actualizar o agregar
  ...
  return (
    <>
      <AnimatePresence>
        ...
        px-6 py-4 rounded-2xl shadow-2xl text-white
        backdrop-blur-md
        border-l-4
            ...
            <div className="flex justify-between items-start gap-6">
              <div>
                ...
                <p className="text-xs opacity-80 mt-1">
                  ...
                  {notificacion.urgente && " • IA detectó urgencia 🚨"}
                </p>
              </div>
